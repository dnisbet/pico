...
class PIOLcd(LcdApi):
    ...
    def __init__(self, rs_pin, enable=None, outbase=None, fourbit=True,
        ...
        self.sm = rp2.StateMachine(0, self._4bit_write, freq=10000,
                        sideset_base=enable, out_base=outbase, pull_thresh=4)
        self.sm.active(1)
        ...
        if self._4bit is True:
            ...
            self.sm.active(0)
            self.sm = rp2.StateMachine(0, self._4bit_write, freq=100000,
                            sideset_base=enable, out_base=outbase, pull_thresh=8)
            self.sm.active(1)
        else:
            # switch the state machine to 8 bit
            self.sm.active(0)
            self.sm = rp2.StateMachine(0, self._8bit_write, freq=1000000,
                            sideset_base=enable, out_base=outbase)
            self.sm.active(1)
